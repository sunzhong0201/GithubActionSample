...
def send_xuangubao(access_token, xuangubaolist):
    # touser 就是 openID
    # template_id 就是模板ID
    # url 就是点击模板跳转的url
    # data就按这种格式写，time和text就是之前{{time.DATA}}中的那个time，value就是你要替换DATA的值
    ...
    body = {
        "touser": openId.strip(),
        "template_id": xuangubao_template_id.strip(),
        "url": "https://weixin.qq.com",
        "data": {
            # "date": {
            #     "value": today_str
            # },
            "region": {
                "value": xuangubaolist[0]
            },
            "weather": {
                "value": xuangubaolist[2]
            },
            "temp": {
                "value": xuangubaolist[1]
            },
            "wind_dir": {
                "value": xuangubaolist[3]
            },
            # "today_note": {
            #     "value": get_daily_love()
            # }
...
def xuangubao_report(this_city):
    # 1.获取access_token
    access_token = get_access_token()
    # 2. 获取天气
    xugubao = get_xugubao()
    weather = get_weather(this_city)
    print(f"选股信息： {xugubao},天气：{weather}")
    # 3. 发送消息
    weather.append(xugubao)
    send_xuangubao(access_token, weather)
